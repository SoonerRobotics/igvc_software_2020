...
from heuristics import travel_cost, path_cost
from rrt import RRT
...
class RRTStar(RRT):
    ...
    def get_nearby_vertices(self, tree, x_init, x_new):
        ...
        L_near = [(path_cost(self.trees[tree].E, x_init, x_near) + travel_cost(x_near, x_new), x_near) for
                  x_near in X_near]
    ...
    def rewire(self, tree, x_new, L_near):
        ...
        for c_near, x_near in L_near:
            ...
            tent_cost = path_cost(self.trees[tree].E, self.x_init, x_new) + travel_cost(x_new, x_near)
    ...
    def connect_shortest_valid(self, tree, x_new, L_near):
        ...
        for c_near, x_near in L_near:
            if c_near + travel_cost(x_near, self.x_goal) < self.c_best and self.connect_to_point(tree, x_near, x_new):
                break
